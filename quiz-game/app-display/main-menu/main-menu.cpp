...
namespace main_menu {
std::array<std::string, 6> choices = {"1. Quick Start", "2. Leaderboard",
                                      "3. New User", "4. Exit", ""};

std::array<std::string, 6> choice_descriptions = {
    " 5 miscellanious questions", " Top scores by different users",
    " Add a new user", " Exit the application", ""};
...
**/
void render_and_operate(int win_y, int win_x, int sub_win_y, int sub_win_x,
                        int main_menu_width) {
  /* Create items */
  ...
  WINDOW *menu_win = newwin(20, 50, win_y, win_x);
  keypad(menu_win, TRUE);

  /* Set main window and sub window */
  set_menu_win(menu, menu_win);
  set_menu_sub(menu, derwin(menu_win, 12, 48, sub_win_y, sub_win_x));
  ...
  print_window(menu_win, 0, 1, main_menu_width, "Main Menu", COLOR_PAIR(1));

  mvwaddch(menu_win, 2, 0, ACS_LTEE);
  mvwhline(menu_win, 2, 1, ACS_HLINE, main_menu_width - 2);
  mvwaddch(menu_win, 2, main_menu_width - 1, ACS_RTEE);
  ...
  while ((c = wgetch(menu_win)) != KEY_F(1)) {
    ...
    case 10: // enter
      ...
      // quick_start::render(win_y, win_x + 50, sub_win_y, sub_win_x);
      std::string s;
      std::cout << "Enter: ";
      std::cin >> s;
      std::cout << s << '\n';
...
void show_choice_and_desc(MENU *menu, int y, int x) {
  ...
  mvprintw(y, x, "%s:\t %s", item_name(cur_item), item_description(cur_item));
