...
const uint64_t &Category::getValueOfId() const noexcept {
  const static uint64_t defaultValue = uint64_t();
  if (id_)
    return *id_;
  return defaultValue;
}
const std::shared_ptr<uint64_t> &Category::getId() const noexcept {
  return id_;
}
void Category::setId(const uint64_t &pId) noexcept {
...
bool Category::validateJsonForUpdate(const Json::Value &pJson,
                                     std::string &err) {
  if (pJson.isMember("id")) {
...
bool Category::validJsonOfField(size_t index, const std::string &fieldName,
