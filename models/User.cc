...
User::User(
  ...
  if (!pMasqueradingVector[3].empty() &&
    ...
    if (!pJson[pMasqueradingVector[3]].isNull()) {
      score_ = std::make_shared<std::string>(
...
const std::shared_ptr<uint64_t> &User::getId() const noexcept { return id_; }
...
void User::updateId(const uint64_t id) { id_ = std::make_shared<uint64_t>(id); }

const std::vector<std::string> &User::insertColumns() noexcept {
  static const std::vector<std::string> inCols = {"name", "password", "score"};
  return inCols;
}
