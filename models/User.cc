...
User::User(
  ...
  if (!pMasqueradingVector[3].empty() &&
    ...
    if (!pJson[pMasqueradingVector[3]].isNull()) {
      score_ =
          std::make_shared<double>(pJson[pMasqueradingVector[3]].asDouble());
    }
  }
}
...
const double &User::getValueOfScore() const noexcept {
  const static double defaultValue = double();
  if (score_)
    return *score_;
  return defaultValue;
}
const std::shared_ptr<double> &User::getScore() const noexcept {
  return score_;
}
void User::setScore(const double &pScore) noexcept {
